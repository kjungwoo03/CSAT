...
def main(args):
    ...
    for attacker_model_idx, exp in enumerate(benchmark.test_stream):
        ...
        for dataset_idx, exp2 in enumerate(benchmark.test_stream):
            ...
            # if dataset_idx > target_model_idx:
                continue
    ...
    for i in range(len(benchmark.test_stream)):
        acc = 0 if total_samples[i] == 0 else original_correct[i]/total_samples[i]
        print(f"\tModel {i+1}: {original_correct[i]}/{total_samples[i]} = {acc:.4f}")
    print("\n--------------------------------")

    print(f"FGSM ASR per models: ")
    for i in range(len(benchmark.test_stream)):
        j=4
        asr = 0 if original_correct[i] == 0 else fgsm_correct[i]/original_correct[i]
        print(f"\tASR from {i+1}th to {j+1}th model: {fgsm_correct[i]}/{original_correct[i]} = {asr:.4f}")
    print("\n--------------------------------")

    print(f"PGD ASR per models: ")
    for i in range(len(benchmark.test_stream)):
        j=4
        asr = 0 if original_correct[i] == 0 else pgd_correct[i]/original_correct[i]
        print(f"\tASR from {i+1}th to {j+1}th model: {pgd_correct[i]}/{original_correct[i]} = {asr:.4f}")
    print("\n--------------------------------")

    print(f"AA ASR per models: ")
    for i in range(len(benchmark.test_stream)):
        j=4
        asr = 0 if original_correct[i] == 0 else auto_correct[i]/original_correct[i]
        print(f"\tASR from {i+1}th to {j+1}th model: {auto_correct[i]}/{original_correct[i]} = {asr:.4f}")
    print("\n--------------------------------")
